...
void BLEClientInterface::update()
{
  ...
  case ClientState::CONNECTING: {
    ...
    if ( service != nullptr ) {
      ...
      Serial.printf( "BLE server '%s' (%s) connected\n", server_name_.c_str(), client_->getPeerAddress().toString().c_str() );
      return;
    }
...
void BLEClientInterface::setProperty( uint8_t id, const std::vector<uint8_t> &data )
{
  ...
  characteristic->writeValue( data.data(), data.size(), false );

  NimBLEUUID uuid = characteristic->getUUID();
  for ( auto &info : characteristics_ ) {
    if ( info.id == uuid ) {
      info.last_write_time = 0;
      info.awaiting_echo = true;
      break;
    }
  }
}

int BLEClientInterface::getLatency() const
{
  int min_latency = INT32_MAX;
  for ( const auto &info : characteristics_ ) {
    if ( info.last_latency_ms >= 0 && info.last_latency_ms < min_latency ) {
      min_latency = info.last_latency_ms;
    }
  }
                                                ...
                                                NimBLERemoteCharacteristic *characteristic ) const
{
  NimBLERemoteCharacteristic::notify_callback callback =
      [this, uuid]( NimBLERemoteCharacteristic *pBLERemoteCharacteristic, uint8_t *pData,
        ...
        for ( auto &info : characteristics_ ) {
          if ( info.id == uuid ) {
            info.last_message = 0;
            info.data.assign( pData, pData + length );
            if ( info.awaiting_echo ) {
              info.last_latency_ms = info.last_write_time;
              info.awaiting_echo = false;
            }
            break;
          }
        }
  ...
  for ( auto &info : characteristics_ ) {
    if ( info.id == uuid ) {
      if ( !info.subscribed ) {
        info.subscribed = characteristic->subscribe( false, callback, true );
      }

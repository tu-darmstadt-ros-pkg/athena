  ...
  for ( uint16_t characteristic_uuid : COMM_PROPERTY_UUIDS ) {
    ...
    NimBLECharacteristic *characteristic = service_->createCharacteristic(
        ...
        NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::READ_ENC | NIMBLE_PROPERTY::READ_AUTHEN |
            ...
            NIMBLE_PROPERTY::WRITE_NR | NIMBLE_PROPERTY::INDICATE );
#else
    NimBLECharacteristic *characteristic = service_->createCharacteristic(
        NimBLEUUID( characteristic_uuid ),
...
void BLEServerInterface::onStatus( NimBLECharacteristic *pCharacteristic, int code )
{
  if ( code == 0 ) {
    last_latency_ms_ = indicate_time_;
  }
}

int BLEServerInterface::getLatency() const { return last_latency_ms_; }

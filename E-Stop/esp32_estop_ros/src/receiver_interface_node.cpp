#include "esp32_estop_ros/receiver_interface_node.hpp"
#include "host_comm.h"
#include <algorithm>
#include <functional>
...
RCLCPP_COMPONENTS_REGISTER_NODE( esp32_estop_ros::ReceiverInterfaceNode )

namespace esp32_estop_ros
{
    ...
    : LifecycleNode( "receiver_interface_node", options )
{
  ...
  declare_readonly_parameter(
      "serial_port", port_, "Serial port to use for communication with the ESP32 E-Stop device" );
  ...
  set_enabled_service_ = create_service<esp32_estop_interface::srv::SetEnabled>(
      "remote_estop/set_enabled",
      [this]( const std::shared_ptr<esp32_estop_interface::srv::SetEnabled::Request> request,
              std::shared_ptr<esp32_estop_interface::srv::SetEnabled::Response> ) {
...
void ReceiverInterfaceNode::setup()
{
  ...
  remote_comm_status_publisher_ = create_publisher<esp32_estop_interface::msg::CommStatus>(
      "remote_estop/remote_comm_status", rclcpp::QoS( 1 ).reliable().transient_local() );

  deadman_comm_status_publisher_ = create_publisher<esp32_estop_interface::msg::CommStatus>(
...
namespace
{
esp32_estop_interface::msg::CommStatus toMsg( const CommStatus &status )
{
  esp32_estop_interface::msg::CommStatus msg;
    ...
    if ( cross_talker_->hasObject() ) {
      ...
      case crosstalk::object_id<EStopReceiverStatus>(): {
        ...
        if ( cross_talker_->readObject( estop_status ) == crosstalk::ReadResult::Success ) {
          esp32_estop_interface::msg::CommStatus remote_msg =
              toMsg( estop_status.remote_status );
          remote_comm_status_publisher_->publish( remote_msg );
          esp32_estop_interface::msg::CommStatus deadman_msg =

...
This repository contains the firmware, shared libraries, and ROS 2 interface for a wireless emergency stop built around the Seeed Studio XIAO ESP32S3, but any other ESP32 should work as well.
...
All nodes share a common communication stack that uses ESP-NOW and BLE for redundant, low-latency signaling.
...
- Dual-channel wireless transport (ESP-NOW, BLE)
...
- **Remote handset (`esp32_estop_sender_firmware`)** – Hosts the physical E-Stop, soft E-Stop, and release buttons, reads the battery voltage, and pushes state updates through `CommInterface`.
- **Receiver (`esp32_estop_receiver_firmware`)** – Listens as the Bluetooth server and ESP-Now peer, merges wireless channels, asserts the relay output on `D3`, and publishes telemetry over USB using the lightweight [CrossTalk](https://github.com/StefanFabian/crosstalk) protocol.
...
- **ROS 2 interface (`esp32_estop_ros` + `esp32_estop_interface`)** – Wraps the CrossTalk datastream in ROS 2 messages (`CommStatus.msg`) and exposes a `SetEnabled` service to bypass the receiver output during testing.
- **Shared firmware library (`esp32_estop_firmware_common`)** – Abstracts the radios, handles property replication, and centralizes MAC configuration inside `include/comm_interface.h`.

### Communication pipeline

1. The remote handset runs `CommInterface` in _client_ mode. Each button state is latched locally and shipped via ESP-NOW datagrams and BLE characteristics.
...
- The receiver defaults to the safe state (output asserted) if no valid packets arrive within ~500 ms for the hard E-Stop.
...
| ----------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------ |
| Seeed XIAO ESP32S3                  | 2     | <https://www.seeedstudio.com/Wio-SX1262-with-XIAO-ESP32S3-p-5982.html>                                                                                                     | 10€         | Sender and Receiver ESP with BLE, ESP NOW. Any other ESP32 with sufficient PINs should work as well.                                                                |
...
| ------------------------------------- | -------------------------------------------------------------- |
| `esp32_estop_sender_firmware/`   | PlatformIO project for the handheld remote with OLED UI.       |
| `esp32_estop_receiver_firmware/` | PlatformIO project for the base station / relay driver.        |
| `esp32_deadman_sender_firmware/`      | PlatformIO project for the optional deadman switch node.       |
| `esp32_estop_firmware_common/`   | Shared library with BLE, ESP-NOW, and CrossTalk helpers. |
| `esp32_estop_interface/`         | ROS 2 interface definitions (messages & services).             |
| `esp32_estop_ros/`               | ROS 2 node that bridges the receiver to the middleware.        |
...
4. **Configure peer addresses and other settings** – See the next section to point each device at the correct MAC addresses.
...
All peer MAC addresses, as well as all other relevant configuration live in `esp32_estop_firmware_common/include/config.h`:
...
- **Custom user interface** – The OLED rendering lives in `esp32_estop_sender_firmware/src/main.cpp`; swap in your display driver or button layout without touching the transport layer.
- **Integrating with other controllers** – Consume the USB CrossTalk stream directly or extend the ROS 2 node (`esp32_estop_ros/src/receiver_interface_node.cpp`) to publish additional diagnostics.

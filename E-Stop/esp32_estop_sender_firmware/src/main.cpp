...
constexpr const char *TITLE = ROBOT_NAME " Remote E-Stop";
...
elapsedMillis last_soft_estop_update_time = 0;
...
{
  Serial.begin( 115200 );
  Serial.println( "Starting " ROBOT_NAME " E-Stop Remote..." );
...
void updateState()
{
  ...
  if ( release_button_pressed && !estop_state && !soft_estop_state ) {
    ...
    if ( estop_state != current_estop_state || last_status_update_time > STATUS_UPDATE_INTERVAL_MS ) {
      last_status_update_time = 0;
      sender.setEStopState( sender.getEStopState() || estop_state );
    }
    if ( ( soft_estop_state && soft_estop_state != current_soft_estop_state ) ||
         last_soft_estop_update_time > SOFT_ESTOP_UPDATE_INTERVAL_MS ) {
      last_soft_estop_update_time = 0;
...
void drawCommState( int x, int y, CommState state, int latency_ms, float rssi )
{
  ...
  std::string text;
  if ( latency_ms >= 0 ) {
    text = latency_ms > 999 ? ">999ms" : std::to_string( latency_ms ) + "ms";
    text += "(" + std::to_string( int( rssi ) ) + " dBm)";
  } else {
    text = std::to_string( int( rssi ) ) + " dBm";
  }
...
void updateDisplay()
{
  ...
  if ( status.comm_status.ble_state != CommState::DISCONNECTED ||
    ...
    display.drawStr( 2, 48, "BLE" );
    drawCommState( 0, 60, status.comm_status.ble_state, status.comm_status.ble_latency_ms,
                   status.comm_status.ble_rssi );
    display.drawStr( 66, 48, "ESP-NOW" );
    drawCommState( 64, 60, status.comm_status.esp_now_state, status.comm_status.esp_now_latency_ms,
                   status.comm_status.esp_now_rssi );
  }
  display.sendBuffer();
}
